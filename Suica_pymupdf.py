...
def parse_suica_history_text(lines):
    ...
    for line in lines:
        ...
        if match:
            groups = [g.strip() if g else '' for g in match.groups()]
            row = [groups[0], groups[1], groups[2], '', '', '', groups[3], groups[4]]
            data.append(row)
    ...
    df['残額'] = df['残額'].str.replace('\\', '', regex=False).str.replace(',', '').astype(int)
...
if __name__ == '__main__':
    sample_pdf_path = os.path.join("sample", "JE80F121120754077_20231016_20240115160118.pdf")
    ...
    if not os.path.exists(sample_pdf_path):
        ...
        if not extracted_data.empty:
            try:
                ...
                extracted_data.to_csv(output_csv_path, index=False, encoding='utf-8-sig')
