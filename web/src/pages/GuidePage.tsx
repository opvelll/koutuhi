      ...
      <GuideSection icon={<CheckCircle2 className="h-5 w-5" />} title="ステップ2：通勤履歴を確認する">
        <NumberedList
          items={[
            ...
            "日付ごとに開いている「運賃内訳」で、交通費に含める利用分だけをチェックします。往復交通費は選択内容に合わせて更新されます。",
            "通勤経路に間違いがないか確認し、必要なら修正します。",
            "「勤務先・社員情報の入力へ」を押して、ステップ3へ進みます。",
          ]}
        />
        <p className="mt-5 border-l-4 border-amber-500 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-950">
          バス利用履歴には乗車・降車停留所が記録されません。該当日は「バス（事業者名）」を仮の経路として確定済みで表示します。必要な場合だけ通勤経路を修正してください。バス運賃は往復交通費へ自動で加算されます。
        </p>
      ...
      <GuideSection icon={<FileSpreadsheet className="h-5 w-5" />} title="ステップ4：Excelを作成・保存する">
        <NumberedList
          items={[
            ...
            "ExcelはA4横・1ページに設定されています。印刷前にプレビューが1ページになっていることを確認します。",
          ]}
        />
      </GuideSection>
